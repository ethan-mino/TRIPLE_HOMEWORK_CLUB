...
public class PlaceController {
    ...
    @GetMapping("") // 모든 장소 조회
    public ResponseEntity<ApiInfoResponse<List<PlaceEntity>>> getAllPlace(){
        ApiInfoResponse<List<PlaceEntity>> apiResponse = new ApiInfoResponse<>();
    ...
    public ResponseEntity<ApiInfoResponse<PlaceEntity>> getPlaceById(@PathVariable("placeId") String placeId){
        ApiInfoResponse<PlaceEntity> apiResponse = new ApiInfoResponse<>();

        PlaceEntity place = placeService.findById(placeId);
    ...
    @PostMapping("")    // 장소 생성
    public ResponseEntity<ApiInfoResponse<PlaceEntity>> createPlace(@AuthenticationPrincipal CustomUserDetails user,
                                                                    @Valid @RequestBody PlaceEntity place,
        ...
        if(createCnt == 1){
            PlaceEntity createdPlace = placeService.findById(place.getId());  // 생성된 place 조회
    ...
    public ResponseEntity<ApiInfoResponse<PlaceEntity>> updatePlaceById(@AuthenticationPrincipal CustomUserDetails user,
        ...
        PlaceEntity queriedPlace = placeService.findById(placeId);
        if(queriedPlace == null){  // 해당 id의 장소가 존재하지 않는 경우
            ...
            if(updateCnt == 1){ // 장소 수정에 성공한 경우
                ApiInfoResponse<PlaceEntity> apiResponse = new ApiInfoResponse<>();
                PlaceEntity updatedPlace = placeService.findById(placeId);  // 수정된 place 조회
